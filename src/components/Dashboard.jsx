...
import { LogOut, RefreshCw, Zap, TrendingUp, TrendingDown, Sun, Moon, CheckCircle } from 'lucide-react'
...
export default function Dashboard({ user, onLogout }) {
  ...
  const handleAcceptKate = async () => {
    ...
    try {
      ...
      if (data.count > 0) {
        setKateMsg(`Accepted ${data.count} invite${data.count > 1 ? 's' : ''} from Kate`)
        speak(`Done. I've accepted ${data.count} calendar invite${data.count > 1 ? 's' : ''} from Kate.`)
      } else {
        setKateMsg('No pending invites from Kate')
      }
    } catch { setKateMsg('Failed to check invites') }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-root)', overflow: 'hidden' }}>
      ...
      <style>{`
        /* ── LIGHT MODE ── */
        [data-theme="light"] {
          --bg-root:#eef4f8; --bg-panel:#ffffff; --bg-panel-alt:#f5f9fc; --bg-hover:#e8f2f8;
          --accent-blue:#0070aa; --accent-cyan:#0095cc; --accent-bright:#0080bb;
          --accent-green:#00966a; --accent-green-dim:#007a55;
          --text-primary:#0d2233; --text-secondary:#2a5570; --text-muted:#6a9ab0;
          --border-dim:rgba(0,120,180,0.18); --border-mid:rgba(0,150,200,0.35); --border-bright:rgba(0,120,180,0.6);
          --glow-sm:0 1px 4px rgba(0,0,0,0.08); --glow-md:0 4px 16px rgba(0,0,0,0.12);
          --glow-btn:0 2px 8px rgba(0,120,180,0.25); --glow-green:0 0 8px rgba(0,150,106,0.3);
        }
      ...
      {/* ── TICKER BAR ── */}
      {tickers.length > 0 && (
        <div style={{ background: 'rgba(0,0,0,0.45)', borderBottom: '1px solid var(--border-dim)', padding: '0 1rem', height: 28, display: 'flex', alignItems: 'center', gap: '1.5rem', overflowX: 'auto', flexShrink: 0 }}>
          {tickers.map(t => (
            <div key={t.symbol} style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', flexShrink: 0 }}>
              ...
              {t.pct !== null && (
                <span style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: '0.65rem', color: t.up ? 'var(--accent-green)' : '#e05c20', display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <span style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: '0.58rem', color: 'var(--text-muted)', marginLeft: 'auto', flexShrink: 0 }}>{timeStr} MT</span>
        </div>
      )}
      ...
      <header style={{ display: 'flex', alignItems: 'center', padding: '0 1.2rem', height: 58, borderBottom: '1px solid var(--border-dim)', background: 'var(--bg-panel)', backdropFilter: 'blur(8px)', flexShrink: 0, gap: '1rem', zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', minWidth: 'max-content' }}>
          ...
          <span style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: '1.1rem', letterSpacing: '0.28em', color: 'var(--accent-bright)', textShadow: '0 0 14px rgba(64,224,255,0.35)' }}>J.A.R.V.I.S.</span>
        </div>

        {/* Center — large clock + world clocks */}
        <div style={{ flex: 1, textAlign: 'center' }}>
          ...
          {/* World clocks */}
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1.2rem', marginTop: '0.25rem' }}>
            {ZONES.map(z => {
              const t = time.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: z.tz })
              return (
                <div key={z.label} style={{ display: 'flex', gap: '0.3rem', alignItems: 'center' }}>
                  ...
                  <span style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: '0.62rem', color: 'var(--text-secondary)' }}>{t}</span>
                </div>
              )
            })}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', minWidth: 'max-content' }}>
          ...
          {/* Kate accept button */}
          <button onClick={handleAcceptKate} disabled={acceptingKate} title="Accept pending invites from Kate"
            style={{ background: 'transparent', border: '1px solid var(--border-dim)', borderRadius: 1, cursor: 'pointer', padding: '0.3rem 0.6rem', display: 'flex', alignItems: 'center', gap: '0.3rem', color: 'var(--text-muted)', fontFamily: 'Rajdhani, sans-serif', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', transition: 'all 0.2s' }}
      ...
      <div style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid var(--border-dim)', padding: '0.45rem 1.2rem', display: 'flex', alignItems: 'center', gap: '0.6rem', flexShrink: 0 }}>
        ...
        <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '0.85rem', color: isSpeaking ? 'var(--accent-bright)' : 'var(--text-secondary)', letterSpacing: '0.04em', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', transition: 'color 0.3s' }}>
          {`${greeting}, ${firstName}. Here's a look at what's on your radar. I'm here to assist.`}
        </div>
        {lastTranscript && <span style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: '0.65rem', color: 'var(--text-muted)', fontStyle: 'italic', flexShrink: 0 }}>"{lastTranscript}"</span>}
      </div>

      {/* ── MAIN GRID ── */}
      <div style={{ flex: 1, overflow: 'auto', padding: '0.7rem', display: 'grid', gap: '0.7rem',
        gridTemplateColumns: 'repeat(12, 1fr)',
        gridTemplateRows: 'minmax(200px,1fr) minmax(240px,1fr) minmax(100px,auto)',
        gridTemplateAreas: `
        ...
        <style>{`
          ...
          @media (max-width: 600px) {
            .dashboard-grid {
              ...
              grid-template-areas:"weather""calendar""gmail""tasks""notes""news""travel" !important;
            }
