...
export default function Dashboard({ user, onLogout }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100dvh', maxWidth:'100vw', overflow:'hidden', background:'var(--bg-root)' }}>
      ...
      <style>{`
        ...
        /* ── DESKTOP GRID — no travel row ── */
        .dash-grid {
          display:grid; gap:0.7rem;
          grid-template-columns:repeat(12,1fr);
          grid-template-rows:minmax(460px,1fr) minmax(360px,1fr);
          grid-template-areas:
            "w w w c c c g g g g g g"
            "t t t t chat chat chat news news news news news";
        }
        @media (max-width:1024px) and (min-width:601px) {
          .dash-grid { grid-template-columns:1fr 1fr !important; grid-template-rows:auto !important;
            grid-template-areas:"w c" "g g" "t chat" "news news" !important; }
        }
        @media (max-width:600px) {
          .dash-grid { grid-template-columns:1fr !important; grid-template-rows:auto !important;
            grid-template-areas:"w""c""g""t""chat""news" !important; }
      ...
      {/* TICKER BAR — centered */}
      {tickers.length > 0 && (
        <div style={{ background:'rgba(0,0,0,0.55)', borderBottom:'1px solid var(--border-dim)', height:34, display:'flex', alignItems:'center', justifyContent:'center', gap:'2.5rem', padding:'0 1rem', overflowX:'auto', flexShrink:0 }}>
          {tickers.map(t => {
            ...
            return (
              <a key={t.symbol} href={`https://finance.yahoo.com/quote/${sym}`} target="_blank" rel="noopener noreferrer"
                ...
                <span style={{ fontFamily:'Share Tech Mono,monospace', fontSize:'0.88rem', color:'#fff' }}>{t.price?(t.symbol.startsWith('^')?Number(t.price).toLocaleString():`$${t.price}`):'—'}</span>
                {t.pct!==null && (
                  <span style={{ fontFamily:'Share Tech Mono,monospace', fontSize:'0.8rem', color:t.up?'var(--accent-green)':'var(--accent-iron)', display:'flex', alignItems:'center', gap:2 }}>
                    {t.up?<TrendingUp size={10}/>:<TrendingDown size={10}/>}{t.up?'+':''}{t.pct}%
                  </span>
                )}
              </a>
            )
      ...
      {/* HEADER — clock truly centered using absolute positioning */}
      <header style={{
        position:'relative',
        ...
        {/* Logo — left, not in flow */}
        <div style={{ display:'flex', alignItems:'center', gap:'0.55rem', flexShrink:0, zIndex:1 }}>
          <Zap size={22} style={{ color:'var(--accent-bright)' }}/>
          <span style={{ fontFamily:'Rajdhani,sans-serif', fontWeight:700, fontSize:'1.55rem', letterSpacing:'0.25em', color:'var(--accent-bright)', textShadow:'0 0 16px rgba(64,224,255,0.4)', whiteSpace:'nowrap' }}>
            J.A.R.V.I.S.
          </span>
        </div>

        {/* Clock — ABSOLUTELY centered in the header */}
        <div style={{ position:'absolute', left:0, right:0, top:0, bottom:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', pointerEvents:'none' }}>
          <div style={{ fontFamily:'Rajdhani,sans-serif', fontWeight:700, fontSize:'2rem', color:'var(--accent-cyan)', letterSpacing:'0.06em', lineHeight:1 }}>
            {timeStr} <span style={{ fontSize:'0.82rem', color:'var(--text-muted)', letterSpacing:'0.1em' }}>MT</span>
          </div>
          <div style={{ fontFamily:'Share Tech Mono,monospace', fontSize:'0.62rem', color:'var(--text-muted)', letterSpacing:'0.08em', marginTop:3 }}>
            {dateStr.toUpperCase()}
          </div>
          <div style={{ display:'flex', gap:'1.3rem', marginTop:'0.22rem' }}>
            {ZONES.map(z => (
              <div key={z.label} style={{ display:'flex', gap:'0.3rem', alignItems:'center' }}>
                ...
                <span style={{ fontFamily:'Share Tech Mono,monospace', fontSize:'0.64rem', color:'var(--text-secondary)' }}>
        ...
        {/* Right controls */}
        <div className="hide-mobile" style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:'0.7rem', flexShrink:0, zIndex:1 }}>
          ...
          <button onClick={()=>setLightMode(l=>!l)}
            style={{ background:'transparent', border:'1px solid var(--border-dim)', borderRadius:'50%', width:36, height:36, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-secondary)', flexShrink:0 }}>
        ...
        {/* Mobile hamburger */}
        <div className="mobile-only" style={{ marginLeft:'auto', display:'flex', alignItems:'center', flexShrink:0, zIndex:1 }}>
      ...
      {/* MAIN GRID — no travel, no bottom bar */}
